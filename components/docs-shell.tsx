...
export function DocsShell({ currentDoc, currentIndex, groups, html, nextDoc, previousDoc, searchItems }: DocsShellProps) {
  ...
  const titles = useMemo(() => new Map(searchItems.map((item) => [item.slug, item.navigationTitle])), [searchItems]);
  const activeGroup = groups.find((group) => group.slugs.includes(currentDoc.slug)) ?? { label: "文档", slugs: [] };
  const sourceUrl = `https://github.com/chogng/zeta-docs/edit/main/${currentDoc.sourcePath}`;
    ...
    <div className="site-shell">
      <header className="topbar">
        <div className="topbar-main">
          ...
          <Link className="brand" href="/docs/getstarted/overview" aria-label="Zeta 文档首页">
          ...
          <div className="top-actions">
            <a className="github-link" href="https://github.com/chogng/zeta-docs" target="_blank" rel="noreferrer">GitHub ↗</a>
      ...
      <main className="content-layout">
        <article className="doc">
          ...
          <footer className="doc-footer">最后核对：{currentDoc.dateApproved}</footer>
        </article>
        ...
        <div className="search-backdrop" role="presentation" onClick={() => setSearchOpen(false)}>
          <div className="search-dialog" role="dialog" aria-modal="true" aria-label="搜索文档" onClick={(event) => event.stopPropagation()}>
            <div className="search-input-row">
              <span>⌕</span>
              <input ref={searchInput} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索功能、设置或操作…" aria-label="搜索内容" />
              <kbd>ESC</kbd>
            </div>
